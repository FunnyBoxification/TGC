...
{ 
    ...
    float freqx = 1.0f+sin(time)*4.0f;
    float freqy = 1.0f+sin(time*1.3f)*4.0f;
    float freqz = 1.0f+sin(time*1.1f)*4.0f;
    float amp = 1.0f+sin(time*1.4)*30.0f;
   
    float f = sin(input.Normal.x*freqx + time) * sin(input.Normal.y*freqy + time) * sin(input.Normal.z*freqz + time);
    input.Position.z += input.Normal.z * amp * f;
    input.Position.x += input.Normal.x * amp * f;
    input.Position.y += input.Normal.y * amp * f;
	}
...
float4 ps_DiffuseMap(PS_DIFFUSE_MAP input) : COLOR0
{
	...
	return daniado == 1 ? finalColor * blanco : finalColor;
}
